...
INPUT_FILE="rand.txt" #assign the input file name here
OUTPUT_FILE="out.txt" #assign the output file name here
Iterations=1  #assign number of iterations
...
if rank == 0:#for the manager 
    ...
    while(t<Iterations): # while for the iterations
        ...
        if rank%2==1:#For Odd Ones
            sendSlice=np.copy(temp[1:lengthPerSlave+1,1:lengthPerSlave+1])
            ...
            if row%2==0:#Odd and Row even ie first row
                ...
                temp[lenght+1,1:lenght+1]=np.copy(sendSlice[0,:])
                COMM_WORLD.Recv([sendSlice,MPI.INT],source=findTop(rank),tag=14)#tag14 receive from Top
                temp[0,1:lenght+1]=np.copy(sendSlice[lengthPerSlave-1,:])
            ...
            temp[lenght+1,0]=np.copy(sendSlice[0,lengthPerSlave-1])
            COMM_WORLD.Recv([sendSlice,MPI.INT],source=findLeft(rank),tag=14)#Left Receive
            temp[1:lenght+1,0]=np.copy(sendSlice[:,lengthPerSlave-1])
            ...
            if row%2==0:#Even and even row ie first row
                ...
                sendSlice=np.copy(temp[1:lengthPerSlave+1,1:lengthPerSlave+1])
                COMM_WORLD.Send([sendSlice,MPI.INT],dest=findTop(rank),tag=14)#Bot Sends to Top               
                COMM_WORLD.Send([sendSlice,MPI.INT],dest=findBot(rank),tag=14)#Top sends to Bot
                
                COMM_WORLD.Recv([sendSlice,MPI.INT],source=findTop(rank),tag=14)#tag14 receive from Top
                temp[0,1:lenght+1]=np.copy(sendSlice[lengthPerSlave-1,:])
                COMM_WORLD.Recv([sendSlice,MPI.INT],source=findBot(rank),tag=14)#tag14 receive from Bot
                temp[lenght+1,1:lenght+1]=np.copy(sendSlice[0,:])
        #print(temp,'is temp for rank',rank,'at iteration',t)
        gameOfLife(temp)
